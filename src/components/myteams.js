...
import { DeleteSvg, TeamsSvg } from '../icons/all';
import { moderateScale } from 'react-native-size-matters';
///////////////////////////////////////////////
const RenderOneTeam = memo(({ teamCoporation, teamPhoto, teamName, teamid }) => {
    ...
    ////////////////////////////////////////////
    const removeOneTeam = useCallback(() => dispatch(Actions.type("setCurrentModalWithNav", {
        key: "alert",
        YesFunc: () => {
            ...
            setTimeout(() => {
                dispatch(Actions.type("setTeamsUser", {
                    type: 'delete', data: {
                        target: teamid,
                        key: 'teamid'
                    }
    ...
    return (<OuterOneView activeOpacity={0.9} isRTL={isRTL}
        ...
        <OuterOneContent isRTL={isRTL} width="100%">
            ...
            <AnyText size={moderateScale(11)} lower color="clr2" lineH={pixel(12)}
                numberOfLines={1} width={"100%"} isRTL={isRTL} ellipsizeMode="tail"
                css={css`flex: 1;max-width: 90%;`}>{teamCoporation}</AnyText>
...
const RenderMeetoorMyTeams = () => {
    ...
    //////////////////////////////////////////////
    const getMyTeams = useCallback(async (first = false, callback) => {
        try {
            if (!isGetMore) return;
            ...
            if (responseData?.length === 0 || responseData?.noMore) {
                ...
                setLoadMore(false);
                callback && callback();
                first && dispatch(Actions.type("setTeamsUser", {
                    type: 'set',
                    data: []
                }));
                return;
            ...
            console.log("getMyTeams ~ lastId", lastId)
    ...
    const onRefresh = useCallback(() => {
        setRefreshing(true);
        getMyTeams(true, () => setRefreshing(false));
    }, []);
    ////////////////////////////////////////////
    useEffect(() => {
        const checker = (DATA_RENDER === undefined);
        if (checker) getMyTeams(true);
    }, []);
    ////////////////////////////////////////////
    const OneTeamMainMemo = useMemo(() => ({ item }) => {
        return (<RenderOneTeam {...item} />);
    }, []);
    const keyExtractor = useCallback(item => item.teamid + "_my_team" + item.teamName, []);
    ...
    const ListHeaderComponent = useCallback(() => <>
        <OuterOneView activeOpacity={1} isRTL={isRTL}
            ...
            css={css`margin: 0;padding: ${pixel(6)};flex-direction: column;
                margin-bottom: ${pixel(8)};border-radius: ${pixel(8)};`}>
            <OuterOneView activeOpacity={1} isRTL={isRTL}
...
export default memo(RenderMeetoorMyTeams);