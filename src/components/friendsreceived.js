...
///////////////////////////////////////////////
const RenderOneReceived = memo(({ username, userPhoto,
    isSecure = true, isOwner = true, fullName, userid }) => {
    ...
    ////////////////////////////////////////////
    const handleFriend = useCallback((type) => {
        setHolder(type);
...
///////////////////////////////////////////////
const RenderMeetoorFriendsReceived = () => {
    const dispatch = useDispatch();
    const { tabsData, messages } = useSelector(state => state.sign.langData);
    const isDark = useSelector(state => state.sign.isDark);
    const isRTL = useSelector(state => state.sign.isRTL);
    const token = useSelector(state => state.sign.token);
    const DATA_RENDER = useSelector(state => state.main.receivedFriends);
    ...
    //////////////////////////////////////////////
    const getReceivedFriends = useCallback(async (first = false, callback) => {
        try {
            if (!isGetMore) return;
            ...
            if (responseData?.length === 0 || responseData?.noMore) {
                ...
                setLoadMore(false);
                callback && callback();
                first && dispatch(Actions.type("setReceivedFriends", {
                    type: 'set',
                    data: []
                }));
                return;
            ...
            // console.log("setRequestFriends ~ lastId", lastId)
            dispatch(Actions.type("setUsersLastId", {
                key: keyArray,
                val: lastId.idlt
            }));
            dispatch(Actions.type("setReceivedFriends", {
                type: first ? 'set' : "push",
                data: response.data
            }));
            ////////////////////////////////////////////////////////
    ...
    const onRefresh = useCallback(() => {
        setRefreshing(true);
        getReceivedFriends(true, () => setRefreshing(false));
    }, []);
    ////////////////////////////////////////////
    useEffect(() => {
        const checker = (DATA_RENDER === undefined);
        if (checker) getReceivedFriends(true);
    }, []);
    /////////////////////////////////////////
    useEffect(() => {
    ...
    ////////////////////////////////////////////
    const OneReceivedFriendMainMemo = useMemo(() => ({ item }) => {
        return (<RenderOneReceived {...item} />);
    }, []);
    const keyExtractor = useCallback(item => item.userid + "_pending_friend" + item.username, []);
    ...
    return (<FlatScroll back={colors[isDark ? "clr1" : "back1"]}
        ...
        renderItem={OneReceivedFriendMainMemo}
        onScrollBeginDrag={() => setScrollBegin(true)}
        onEndReached={() => {
            scrollBegin && getReceivedFriends();
        }}
...
export default memo(RenderMeetoorFriendsReceived);