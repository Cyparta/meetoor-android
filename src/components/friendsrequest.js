...
///////////////////////////////////////////////
const RenderOnePending = memo(({ username, userPhoto, isSecure = true, isOwner = true, fullName }) => {
...
///////////////////////////////////////////////
const RenderMeetoorFriendsRequest = () => {
    const dispatch = useDispatch();
    const { tabsData, messages } = useSelector(state => state.sign.langData);
    const isDark = useSelector(state => state.sign.isDark);
    const isRTL = useSelector(state => state.sign.isRTL);
    const token = useSelector(state => state.sign.token);
    const DATA_RENDER = useSelector(state => state.main.requestFriends);
    ...
    //////////////////////////////////////////////
    const getPendingFriends = useCallback(async (first = false, callback) => {
        try {
            if (!isGetMore) return;
            ...
            if (responseData?.length === 0 || responseData?.noMore) {
                ...
                setLoadMore(false);
                callback && callback();
                first && dispatch(Actions.type("setRequestFriends", {
                    type: 'set',
                    data: []
                }));
                return;
    ...
    const onRefresh = useCallback(() => {
        setRefreshing(true);
        getPendingFriends(true, () => setRefreshing(false));
    }, []);
    ////////////////////////////////////////////
    useEffect(() => {
        const checker = (DATA_RENDER === undefined);
        if (checker) getPendingFriends(true);
    }, []);
    /////////////////////////////////////////
    useEffect(() => {
    ...
    ////////////////////////////////////////////
    const OnePendingFriendMainMemo = useMemo(() => ({ item }) => {
        return (<RenderOnePending {...item} />);
    }, []);
    ...
    return (<FlatScroll back={colors[isDark ? "clr1" : "back1"]}
        ...
        renderItem={OnePendingFriendMainMemo}
        onScrollBeginDrag={() => setScrollBegin(true)}
        onEndReached={() => {
            scrollBegin && getPendingFriends();
        }}
...
export default memo(RenderMeetoorFriendsRequest);